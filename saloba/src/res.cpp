...
gasal_res_t *gasal_res_new_host(uint32_t max_n_alns, Parameters *params)
{
	...
	CHECKCUDAERROR(cudaHostAlloc(&(res->max_off), max_n_alns * sizeof(int32_t),cudaHostAllocDefault));
	CHECKCUDAERROR(cudaHostAlloc(&(res->gscore), max_n_alns * sizeof(int32_t),cudaHostAllocDefault));
	CHECKCUDAERROR(cudaHostAlloc(&(res->max_ie), max_n_alns * sizeof(int32_t),cudaHostAllocDefault));
...
gasal_res_t *gasal_res_new_device(gasal_res_t *device_cpy)
{
	...
	CHECKCUDAERROR(cudaMemcpy(&(d_c->max_off), &(device_cpy->max_off), sizeof(int32_t*), cudaMemcpyHostToDevice));
	CHECKCUDAERROR(cudaMemcpy(&(d_c->gscore), &(device_cpy->gscore), sizeof(int32_t*), cudaMemcpyHostToDevice));
	CHECKCUDAERROR(cudaMemcpy(&(d_c->max_ie), &(device_cpy->max_ie), sizeof(int32_t*), cudaMemcpyHostToDevice));
...
gasal_res_t *gasal_res_new_device_cpy(uint32_t max_n_alns, Parameters *params)
{
	...
	CHECKCUDAERROR(cudaMalloc(&(res->max_off), max_n_alns * sizeof(int32_t)));
	CHECKCUDAERROR(cudaMalloc(&(res->gscore), max_n_alns * sizeof(int32_t)));
	CHECKCUDAERROR(cudaMalloc(&(res->max_ie), max_n_alns * sizeof(int32_t)));
...
void gasal_res_destroy_host(gasal_res_t *res) 
{
	...
	if (res->max_off != NULL) CHECKCUDAERROR(cudaFreeHost(res->max_off));
	if (res->gscore != NULL) CHECKCUDAERROR(cudaFreeHost(res->gscore));
	if (res->max_ie != NULL) CHECKCUDAERROR(cudaFreeHost(res->max_ie));
...
void gasal_res_destroy_device(gasal_res_t *device_res, gasal_res_t *device_cpy) 
{
	...
	if (device_cpy->max_off != NULL) CHECKCUDAERROR(cudaFree(device_cpy->max_off));
	if (device_cpy->gscore != NULL) CHECKCUDAERROR(cudaFree(device_cpy->gscore));
	if (device_cpy->max_ie != NULL) CHECKCUDAERROR(cudaFree(device_cpy->max_ie));
