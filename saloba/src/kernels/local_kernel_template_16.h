#ifndef __LOCAL_KERNEL_TEMPLATE16__
#define __LOCAL_KERNEL_TEMPLATE16__
...
enum subwarp_16{
	threads_per_pair_16 = 16,
	seqs_per_block_16 = 32 * threadIdx_y / threads_per_pair_16,
};

//template <typename T, typename S, typename B>
__global__ void gasal_local_kernel_16(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, int n_tasks, short2 *device_bottom, int *dev_h0, int band_width, int z_drop)
{

	//return;

	int32_t seqid = blockIdx.x * blockDim.y * (blockDim.x / threads_per_pair_16) + threadIdx.y * (blockDim.x / threads_per_pair_16) + threadIdx.x / threads_per_pair_16;
	int32_t tid = threadIdx.x % threads_per_pair_16;
	if(seqid >= n_tasks) return;
	int32_t seqId_block = threadIdx.y * blockDim.x / threads_per_pair_16 + threadIdx.x / threads_per_pair_16;
	...
	int32_t i;
	int32_t k, m, l; //[remove i->m]
	int32_t e;
	int32_t subScore;
	//----------------------------------------
	int32_t full_chunk_num = target_batch_regs / threads_per_pair_16; //[remove]
	int32_t last_chunk_height = target_batch_regs % threads_per_pair_16; //[remove]
	int32_t chunk_idx; 
	int32_t step;
	int32_t col_block;
	int32_t col_cell;

	int32_t step_q;	//[remove]
	int32_t step_r;	//[remove]
	int32_t offset_partial_bottom; //[remove]
	int32_t offset_device_bottom; //[remove]

	//const int band_width = w;//[banded]
	int32_t global_row = 0;
	int32_t global_col = 0;
	int32_t band_start = 0;
	int32_t band_end = 0;
	int32_t local_start = 0;
	int32_t local_end = 0;

	int32_t h0 = dev_h0[seqid];
	bool is_end_last = 0;

	__shared__ short2 partial_bottom[seqs_per_block_16][threads_per_pair_16 * 2 * 8];
	__shared__ uint32_t bases[seqs_per_block_16][threads_per_pair_16 * 2];

	__shared__ int row_max[seqs_per_block_16][threads_per_pair_16 * 8];
	__shared__ int row_max_col[seqs_per_block_16][threads_per_pair_16 * 8];

	__shared__ short4 thread_max[seqs_per_block_16][threads_per_pair_16 + 1]; 
	__shared__ int thread_max_off[seqs_per_block_16][threads_per_pair_16 + 1];

	__shared__ short2 thread_g[seqs_per_block_16][threads_per_pair_16 + 1];
	//----------------------------------------
	//initialize-----------------------------
	//[can be optimized. we know h0, then means know number of interatino]
	short2 initHD = make_short2(0, 0);
	for(i = 0; i < (int)(read_len / threads_per_pair_16) + 2; ++i){
		int h = h0 - _cudaGapOE - _cudaGapExtend * (i*threads_per_pair_16 + tid);
		h = h > 0 ? h : 0;
		initHD.x = h;
		device_bottom[seqid * MAX_QUERY_LEN + i * threads_per_pair_16 + tid] = initHD; //[i*threads_per_pair_16 + tid]
	}
	short4 SRCE;
	thread_max[seqId_block][tid] = make_short4(0, 0, 0, threads_per_pair_16);
	if(tid ==0){
		thread_max[seqId_block][0].x = h0;
		thread_max[seqId_block][0].y = -1;
		thread_max[seqId_block][0].z = -1;
		thread_max[seqId_block][0].w = threads_per_pair_16;
	}
	...
	for(chunk_idx = 0; chunk_idx < full_chunk_num; ++chunk_idx){
		...
		for(int c = 0; c < 8; ++c){
			...
			partial_bottom[seqId_block][(tid + threads_per_pair_16)*8 + c] = device_bottom[seqid*MAX_QUERY_LEN +  (tid + threads_per_pair_16)*8 + c];
		}
		//-------------------------	
		global_row = 8 * threads_per_pair_16 * chunk_idx + 8 * tid;
		global_col = 0;
		...
		__syncwarp();
		//-------------------------
		register uint32_t gpac =packed_target_batch[packed_target_batch_idx + threads_per_pair_16 * chunk_idx + tid];//load 8 packed bases from target_batch sequence
		for(step = 0; step < threads_per_pair_16 + query_batch_regs -1;){
			if(tid <= step && step < tid + query_batch_regs /*&& band_start <= global_row + 7*/){
				//-------------initialize-------------------------
				//read ref and qeury regs
				//fill 8x8cell
				col_cell = 0;
				global_col = 8*col_block;
				band_start = (global_col) - band_width; 
				band_end = (global_col) + band_width;
				if(!(band_end + 7 < global_row)){
				register uint32_t rpac = bases[seqId_block][col_block % (2*threads_per_pair_16)];//load 8 bases from query_batch sequence
				#pragma unroll 8
				for (k = 28; k >= 0; k -= 4, ++col_cell, ++global_col, ++band_start, ++band_end) {
					//--------------decide row range--------------
					if(band_end < global_row /*|| global_row + 7 < band_start*/) {
						p[1] = partial_bottom[seqId_block][(col_block % (threads_per_pair_16 * 2))*8 + col_cell].x;
						continue;
					}
					if(global_row + 7 < band_start){
						break;
					}
					local_start = band_start - global_row;
					local_end = band_end - global_row;
					...
					HD = partial_bottom[seqId_block][(col_block % (threads_per_pair_16 * 2))*8 + col_cell];
					h[0] = HD.x;
					e = HD.y;
					//--------------------------------------------
					#pragma unroll 8
					...
					//for (l = 28 - 4*local_start, m = 1 + local_start; m < 1 + (local_end + 1); l -= 4, m++) {
						...
						h[m] = max(h[m], 0); 
						f[m] = max(tmp_hm- _cudaGapOE, f[m] - _cudaGapExtend); 
						if(local_start <= (m-1) && (m-1) <= local_end){
							e = max(tmp_hm- _cudaGapOE, e - _cudaGapExtend);
						}
						else{
							//do nothing
					...
					for(i = 1; i < 9; ++i){
						p[i] = h[i-1];
						row_max[seqId_block][tid * 8 + i - 1] = row_max[seqId_block][tid * 8 + i - 1] > h[i] ? row_max[seqId_block][tid * 8 + i - 1] : h[i];
						row_max_col[seqId_block][tid * 8 + i - 1] = row_max[seqId_block][tid * 8 + i - 1] > h[i] ? row_max_col[seqId_block][tid * 8 + i - 1] : global_col;
					}
					//----------save intermediate values------------
					//save result on shared_memory[~][col]
					HD.x = h[m-1];
					HD.y = e;
					partial_bottom[seqId_block][(col_block % (threads_per_pair_16 * 2))*8 + col_cell] = HD;
			...
			++step;
			step_q = (step) / threads_per_pair_16;
			step_r = (step) % threads_per_pair_16;
			if(step_q >= 2 && step_r == 0){
				offset_partial_bottom = (step_q % 2) * threads_per_pair_16;
				offset_device_bottom = (step_q -2) * threads_per_pair_16;
				...
				//---------------------------------
				for(i = 0; i < 8; ++i){
					partial_bottom[seqId_block][(offset_partial_bottom + tid)*8 + i] = device_bottom[seqid * MAX_QUERY_LEN + offset_device_bottom + threads_per_pair_16*2*8 + i];
				}
				//--------------------------------
			}
			if(step % (threads_per_pair_16) == 0){
				//((step/threads_per_pair_16)/2)
				bases[seqId_block][((step/threads_per_pair_16)%2)*threads_per_pair_16 + tid] = packed_query_batch[packed_query_batch_idx + step + tid];
			}
			__syncwarp();
		...
		for(i = 1; i < threads_per_pair_16; i = i * 2){
		...
		//kogge-stone algorithm for max_off. target idx is 1<= < threads_per_pair_16+1
		for(i = 1; i < threads_per_pair_16 + 1; i = i * 2){
			int bigger = 0;
			if(i-1 <= tid){
		...
		if(tid == 0){
			//max
			int idx = thread_max[seqId_block][threads_per_pair_16-1].w == threads_per_pair_16 ? threads_per_pair_16 -1 : thread_max[seqId_block][threads_per_pair_16-1].w;
		...
		if(thread_max[seqId_block][0].w < threads_per_pair_16) break;
		...
		if(tid == 0){
			int idx = thread_max[seqId_block][threads_per_pair_16-1].w == threads_per_pair_16 ? threads_per_pair_16 -1 : thread_max[seqId_block][threads_per_pair_16-1].w;
			thread_max[seqId_block][0] = thread_max[seqId_block][idx];
