...
#define CORE_LOCAL_COMPUTE() \
		...
		int32_t tmp_hm = p[seqId_block][tid][m] ? p[seqId_block][tid][m] + subScore : 0; \
		h[seqId_block][tid][m] = max(tmp_hm, f[seqId_block][tid][m]); \
		h[seqId_block][tid][m] = max(h[seqId_block][tid][m], e); \
		h[seqId_block][tid][m] = max(h[seqId_block][tid][m], 0); \
		f[seqId_block][tid][m] = max(tmp_hm- _cudaGapOE, f[seqId_block][tid][m] - _cudaGapExtend); \
		e = max(tmp_hm- _cudaGapOE, e - _cudaGapExtend); \
		p[seqId_block][tid][m] = h[seqId_block][tid][m-1]; \
...
#define full_chunk_num (target_batch_regs[seqId_block] / threads_per_pair)
#define last_chunk_height (target_batch_regs[seqId_block] % threads_per_pair)
...
__global__ void gasal_local_kernel(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, int n_tasks, short2 *device_bottom, int *dev_h0, int band_width, int z_drop)
{
	...
	int seqid = blockIdx.x * blockDim.y * (blockDim.x / threads_per_pair) + threadIdx.y * (blockDim.x / threads_per_pair) + threadIdx.x / threads_per_pair;
	int tid = threadIdx.x % threads_per_pair;
	if(seqid >= n_tasks) return;
	int seqId_block = threadIdx.y * blockDim.x / threads_per_pair + threadIdx.x / threads_per_pair;
	//--------------------------------------------------------------------------------------------------------------------------
	...
	__shared__ uint32_t packed_target_batch_idx[seqs_per_block];
	__shared__ uint32_t packed_query_batch_idx[seqs_per_block];
	
	if(tid == 0){
		packed_target_batch_idx[seqId_block] = target_batch_offsets[seqid] >> 3;
		packed_query_batch_idx[seqId_block] = query_batch_offsets[seqid] >> 3;
	}

	uint32_t read_len = query_batch_lens[seqid];
	uint32_t ref_len = target_batch_lens[seqid];

	__shared__ uint32_t query_batch_regs[seqs_per_block];
	__shared__ uint32_t target_batch_regs[seqs_per_block];

	if(tid == 0){
		query_batch_regs[seqId_block] = (read_len >> 3) + (read_len&7 ? 1 : 0);
		target_batch_regs[seqId_block] = (read_len >> 3) + (read_len&7 ? 1 : 0);
	}
	...
	__shared__ int32_t h[seqs_per_block][threads_per_pair][9];
	__shared__ int32_t f[seqs_per_block][threads_per_pair][9];
	__shared__ int32_t p[seqs_per_block][threads_per_pair][9];
	...
	int step_q;
	int step_r;
	int offset_partial_bottom;
	int offset_device_bottom;
	...
	for(i = 0; i < (read_len / threads_per_pair) + 2; ++i){
		int h = h0 - _cudaGapOE - _cudaGapExtend * (i*threads_per_pair + tid);
		h = h > 0 ? h : 0;
		initHD.x = h;
		device_bottom[seqid * MAX_QUERY_LEN + i * threads_per_pair + tid] = initHD; //[i*threads_per_pair + tid]
	}

	short4 initM = make_short4(0, 0, 0, threads_per_pair);
	short4 SRCE;
	thread_max[seqId_block][tid] = initM;
	if(tid ==0){
	...
	for(chunk_idx = 0; chunk_idx < full_chunk_num; ++chunk_idx){
		...
		is_end_last = 0;
		for (m = 0; m < 9; m++) {
            h[seqId_block][tid][m] = 0;
            f[seqId_block][tid][m] = 0;
            p[seqId_block][tid][m] = 0;
			row_max[m] = 0;
			row_max_col[m] = 0;
        }
		//----------------------------
		for(i = 0; i < 8; ++i){
			partial_bottom[seqId_block][tid*8 + i] = device_bottom[seqid*MAX_QUERY_LEN +  tid*8 + i];
			partial_bottom[seqId_block][(tid + threads_per_pair)*8 + i] = device_bottom[seqid*MAX_QUERY_LEN +  (tid + threads_per_pair)*8 + i];
		}
		//-------------------------	
		...
		if(!(band_end < global_row || global_row + 7 < band_start)) {
			...
			for(i = local_start; i < local_end + 1; ++i){
				p[seqId_block][tid][i+1] = h0 - _cudaGapOE - _cudaGapExtend*(global_row + i - 1);
				p[seqId_block][tid][i+1] = p[seqId_block][tid][i+1] > 0 ? p[seqId_block][tid][i+1] : 0;
			}
		}
		if(chunk_idx == 0 && tid == 0){
			p[seqId_block][tid][1] = h0;
		}
		//-------------------------
		for(step = 0; step < threads_per_pair + query_batch_regs[seqId_block] -1;){
			if(tid <= step && step < tid + query_batch_regs[seqId_block]){
				//-------------initialize-------------------------
				//if(band_end + 7 < global_row || band_start > global_row + 7) continue;
				//read ref and qeury regs
				register uint32_t gpac =packed_target_batch[packed_target_batch_idx[seqId_block] + threads_per_pair * chunk_idx + tid];//load 8 packed bases from target_batch sequence
				register uint32_t rpac =packed_query_batch[packed_query_batch_idx[seqId_block] + col_block];//load 8 bases from query_batch sequence
				//fill 8x8cell
				col_cell = 0;
				for (k = 28; k >= 0; k -= 4, ++col_cell, ++global_col, ++band_start, ++band_end) {
					//--------------decide row range--------------
					if(band_end < global_row || global_row + 7 < band_start) {
						p[seqId_block][tid][1] = partial_bottom[seqId_block][(col_block % (threads_per_pair * 2))*8 + col_cell].x;
						continue;
					}
					local_start = band_start - global_row;
					local_end = band_end - global_row;
...
//#pragma unroll 8
					for (l = 28 - 4*local_start, m = 1 + local_start; m < 1 + (local_end + 1); l -= 4, m++) {
						CORE_LOCAL_COMPUTE();
						row_max[m] = row_max[m] > h[seqId_block][tid][m] ? row_max[m] : h[seqId_block][tid][m];
						row_max_col[m] = row_max[m] > h[seqId_block][tid][m] ? row_max_col[m] : global_col;
					}
					p[seqId_block][tid][m] = h[seqId_block][tid][m-1];
					//----------save intermediate values------------
					//save result on shared_memory[~][col]
					HD.x = h[seqId_block][tid][m-1];
					HD.y = e;
			...
			if(step_q >= 2 && step_r == 0){
				...
				for(i = 0; i < 8; ++i){
					device_bottom[seqid * MAX_QUERY_LEN + offset_device_bottom + i] = partial_bottom[seqId_block][(offset_partial_bottom + tid)*8 + i];
				}
				//---------------------------------
				for(i = 0; i < 8; ++i){
					partial_bottom[seqId_block][(offset_partial_bottom + tid)*8 + i] = device_bottom[seqid * MAX_QUERY_LEN + offset_device_bottom + threads_per_pair*2*8 + i];
				}
				//--------------------------------
			}
			__syncwarp();
		...
		for(m = 1; m < 9; ++m){
			if(row_max[m] > thread_max[seqId_block][tid+1].x){
				thread_max[seqId_block][tid+1].x = row_max[m];
				thread_max[seqId_block][tid+1].y = global_row + m -1;
				thread_max[seqId_block][tid+1].z = row_max_col[m];
			}
		}
		...
		for(m = 1; m < 9; ++m){
			...
			if(local_start <= (m-1) && (m-1) <= local_end && is_end_last){
				if(h[seqId_block][tid][m] >= thread_g[seqId_block][tid+1].x && (global_row+m-1)<ref_len){
					thread_g[seqId_block][tid+1].x = h[seqId_block][tid][m];
			...
			if(row_max[m] > thread_max[seqId_block][tid].x){
				thread_max[seqId_block][tid].x = row_max[m];
				thread_max[seqId_block][tid].y = global_row + m - 1;
				thread_max[seqId_block][tid].z = row_max_col[m];

				i = (global_row + m -1) - row_max_col[m];
				if(i < 0) i = -i;
				thread_max_off[seqId_block][tid+1] = thread_max_off[seqId_block][tid+1] > i ? thread_max_off[seqId_block][tid+1] : i;
			}
			//z-drop
			//thread_max[seqId_block][tid].y,z ~ (global_row+m-1)~row_max_col[m];
			if((thread_max[seqId_block][tid].y - (global_row + m -1)) > (thread_max[seqId_block][tid].z - row_max_col[m])){
				if(thread_max[seqId_block][tid].x - row_max[m] - ((thread_max[seqId_block][tid].y - (global_row + m -1)) - (thread_max[seqId_block][tid].z - row_max_col[m]))*_cudaGapExtend > z_drop){
					thread_max[seqId_block][tid].w = tid;
					break;
				}
			}
			else{
				if(thread_max[seqId_block][tid].x - row_max[m] - ((thread_max[seqId_block][tid].z - row_max_col[m]) - (thread_max[seqId_block][tid].y - (global_row + m -1)))*_cudaGapExtend > z_drop){
					thread_max[seqId_block][tid].w = tid;
					break;
				}
			}
		...
		for(i = 1; i < threads_per_pair; i = i * 2){
			m = 0;
			if(i <= tid){
				m = thread_max[seqId_block][tid-i].w < thread_max[seqId_block][tid].w ? thread_max[seqId_block][tid-i].w : thread_max[seqId_block][tid].w;
				__syncwarp();
				thread_max[seqId_block][tid].w = m;
			}
			else{
				__syncwarp();	
			}
		...
		for(i = 1; i < threads_per_pair + 1; i = i * 2){
			m = 0;
			if(i-1 <= tid){
				m = thread_max_off[seqId_block][tid + 1 -i] < thread_max_off[seqId_block][tid + 1 ] ? thread_max_off[seqId_block][tid + 1 ] : thread_max_off[seqId_block][tid + 1 -i];
				__syncwarp();
				thread_max_off[seqId_block][tid + 1 ] = m;
			}
			else{
				__syncwarp();	
			}
		...
		if(tid == 0){
			//max
			i = thread_max[seqId_block][threads_per_pair-1].w == threads_per_pair ? threads_per_pair -1 : thread_max[seqId_block][threads_per_pair-1].w;
			thread_max[seqId_block][0].x = thread_max[seqId_block][i].x;
			thread_max[seqId_block][0].y = thread_max[seqId_block][i].y;
			thread_max[seqId_block][0].z = thread_max[seqId_block][i].z;
			thread_max[seqId_block][0].w = thread_max[seqId_block][i].w;

			thread_max_off[seqId_block][0] = thread_max_off[seqId_block][i + 1] > thread_max_off[seqId_block][0] ? thread_max_off[seqId_block][i + 1] : thread_max_off[seqId_block][0];
			thread_g[seqId_block][0].y = thread_g[seqId_block][i + 1].x >= thread_g[seqId_block][0].x ? thread_g[seqId_block][i + 1].y : thread_g[seqId_block][0].y;
			thread_g[seqId_block][0].x = thread_g[seqId_block][i + 1].x >= thread_g[seqId_block][0].x ? thread_g[seqId_block][i + 1].x : thread_g[seqId_block][0].x; 
		}
		thread_max_off[seqId_block][tid+1] = 0;
		thread_g[seqId_block][tid+1] = initHD;
		__syncwarp();
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
		...
		//-------------------------
		__syncwarp();
		for(step = 0; step < last_chunk_height + query_batch_regs[seqId_block] -1;){
			//if(max_info[seqId_block][3]) break;
			if(thread_max[seqId_block][0].w < threads_per_pair) break;
			if(tid <= step && step < tid + query_batch_regs[seqId_block] && tid < last_chunk_height){
				//printf("last chunk\n");
				//read ref and qeury regs
				register uint32_t gpac =packed_target_batch[packed_target_batch_idx[seqId_block] + threads_per_pair * chunk_idx + tid];//load 8 packed bases from target_batch sequence
				register uint32_t rpac =packed_query_batch[packed_query_batch_idx[seqId_block] + col_block];//load 8 bases from query_batch sequence
				...
				for (k = 28; k >= 0; k -= 4, ++col_cell, ++global_col, ++band_start, ++band_end) {
					...
					if(local_end > 7) local_end = 7;
					if(seqid == 28 && tid == 2){
						//printf("[%d] %d %d\n", global_col, local_start, local_end);
					}
					//--------------------------------------------
					uint32_t rbase = (rpac >> k) & 15;//get a base from query_batch sequence
					//-----load intermediate values--------------
...
//#pragma unroll 8
			...
			if(step_q >= 2 && step_r == 0){
				...
				offset_device_bottom = (offset_device_bottom + tid) * 8;
				for(i = 0; i < 8; ++i){
					device_bottom[seqid * MAX_QUERY_LEN + offset_device_bottom + i] = partial_bottom[seqId_block][(offset_partial_bottom + tid)*8 + i]; //[flag] is this needed?
				}
				//---------------------------------
				for(i = 0; i < 8; ++i){
					partial_bottom[seqId_block][(offset_partial_bottom + tid)*8 + i] = device_bottom[seqid * MAX_QUERY_LEN + offset_device_bottom + threads_per_pair*2*8 + i];
