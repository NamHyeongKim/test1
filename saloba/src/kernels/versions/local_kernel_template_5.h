...
#define packed_target_batch_idx_L ((uint32_t)target_batch_offsets[seqid] >> 3)
#define packed_query_batch_idx_L ((uint32_t)query_batch_offsets[seqid] >> 3)
#define read_len_L ((uint32_t)query_batch_lens[seqid])
#define ref_len_L ((uint32_t)target_batch_lens[seqid])
#define query_batch_regs_L ((uint32_t)(read_len_L >> 3) + (read_len_L&7 ? 1 : 0))
#define target_batch_regs_L ((uint32_t)(ref_len_L >> 3) + (ref_len_L&7 ? 1 : 0))

#define full_chunk_num (target_batch_regs_L / threads_per_pair)
#define last_chunk_height (target_batch_regs_L % threads_per_pair)
#define offset_partial_bottom ((step_q % 2) * threads_per_pair)
#define offset_device_bottom ((((step_q -2) * threads_per_pair)+tid)*8)
...
__global__ void gasal_local_kernel(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, int n_tasks, short2 *device_bottom, int *dev_h0, int band_width, int z_drop)
{
	...
	//--------------------------------------------------------------------------------------------------------------------------
	//uint32_t packed_target_batch_idx_L = ((uint32_t)target_batch_offsets[seqid] >> 3); //starting index of the target_batch sequence
	//uint32_t packed_query_batch_idx_L = ((uint32_t)query_batch_offsets[seqid] >> 3);//starting index of the query_batch sequence
	//uint32_t read_len_L = ((uint32_t)query_batch_lens[seqid]);
	//uint32_t ref_len_L = ((uint32_t)target_batch_lens[seqid]);
	//uint32_t query_batch_regs_L = ((uint32_t)(read_len_L >> 3) + (read_len_L&7 ? 1 : 0));//number of 32-bit words holding query_batch sequence
	//uint32_t target_batch_regs_L = ((uint32_t)(ref_len_L >> 3) + (ref_len_L&7 ? 1 : 0));//number of 32-bit words holding target_batch sequence
	//--------------------------------------------------------------------------------------------------------------------------
	...
	int16_t subScore;
	//----------------------------------------
	//uint16_t full_chunk_num = target_batch_regs_L / threads_per_pair; //[remove]
	//uint16_t last_chunk_height = target_batch_regs_L % threads_per_pair; //[remove]
	uint16_t chunk_idx; 
	uint16_t step;
	uint32_t col_block;
	uint32_t col_cell;
	...
	for(i = 0; i < (int)(read_len_L / threads_per_pair) + 2; ++i){
	...
	for(chunk_idx = 0; chunk_idx < full_chunk_num; ++chunk_idx){
		...
		//------------------------
		bases[seqId_block][tid] = packed_query_batch[packed_query_batch_idx_L + tid];
		//bases[seqId_block][threads_per_pair + tid] = packed_query_batch[packed_query_batch_idx_L + threads_per_pair + tid];
		__syncwarp();
		//-------------------------
		register uint32_t gpac =packed_target_batch[packed_target_batch_idx_L + threads_per_pair * chunk_idx + tid];//load 8 packed bases from target_batch sequence
		for(step = 0; step < threads_per_pair + query_batch_regs_L -1;){
			if(tid <= step && step < tid + query_batch_regs_L /*&& band_start <= global_row + 7*/){
				//-------------initialize-------------------------
...
//#pragma unroll 8
			...
			if(step_q >= 2 && step_r == 0){
				...
				//offset_device_bottom = (((step_q -2) * threads_per_pair)*8);
			...
			if(step % (threads_per_pair) == 0){
				...
				bases[seqId_block][((step/threads_per_pair)%2)*threads_per_pair + tid] = packed_query_batch[packed_query_batch_idx_L + step + tid];
			}
			__syncwarp();
		...
		//if(step_r != 0){
			++step_q;
			//offset_partial_bottom = (step_q % 2) * threads_per_pair;
			//offset_device_bottom = ((((step_q -2) * threads_per_pair)+tid)*8);
			//offset_device_bottom = (offset_device_bottom + tid) * 8;
			for(i = 0; i < 8; ++i){
				device_bottom[seqid * MAX_QUERY_LEN + offset_device_bottom + i] = partial_bottom[seqId_block][(offset_partial_bottom + tid)*8 + i];
			}
		...
		for(m = 1; m < 9; ++m){
			...
			if(local_start <= (m-1) && (m-1) <= local_end && is_end_last){
				if(h[m] >= thread_g[seqId_block][tid+1].x && (global_row+m-1)<ref_len_L){
		...
		__syncwarp();
		register uint32_t gpac =packed_target_batch[packed_target_batch_idx_L + threads_per_pair * chunk_idx + tid];//load 8 packed bases from target_batch sequence
		for(step = 0; step < last_chunk_height + query_batch_regs_L -1;){
			//if(max_info[seqId_block][3]) break;
			if(thread_max[seqId_block][0].w < threads_per_pair) break;
			if(tid <= step && step < tid + query_batch_regs_L && tid < last_chunk_height /*&& band_start <= global_row + 7*/){
				//printf("last chunk\n");
				//read ref and qeury regs
				register uint32_t rpac =bases[seqId_block][col_block %(2*threads_per_pair)];//load 8 bases from query_batch sequence
				//fill 8x8cell
...
//#pragma unroll 8
			...
			if(step % (threads_per_pair) == 0){
				bases[seqId_block][((step/threads_per_pair)%2)*threads_per_pair + tid] = packed_query_batch[packed_query_batch_idx_L + step + tid];
				//bases[seqId_block][threads_per_pair + tid] = packed_query_batch[packed_query_batch_idx_L + step + threads_per_pair + tid];
			}
			__syncwarp();
		}
		for(m = 1; m < 9; ++m){
