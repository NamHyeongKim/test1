...
#define CORE_LOCAL_COMPUTE() \
		...
		maxXY_y = (maxHH < h[m]) ? (chunk_idx * threads_per_pair * 8+ tid * 8) + (m-1) : maxXY_y; \
		maxHH = (maxHH < h[m]) ? h[m] : maxHH; \
		p[m] = h[m-1]; \
...
__global__ void gasal_local_kernel(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, int n_tasks, short2 *device_bottom, int *dev_h0, int band_width, int z_drop)
{
	...
	//printf("call\n");
	//printf("%d\n", MAX_QUERY_LEN);
	//short2 *device_bottom = (short2 *)device_bottoms;
	...
	int32_t subScore;
	//----------------------------------------
	uint32_t full_chunk_num = target_batch_regs / threads_per_pair;
	uint32_t last_chunk_height = target_batch_regs % threads_per_pair;
	uint32_t chunk_idx;
	uint32_t step;
	uint32_t col_block;
	uint32_t col_cell;
	...
	// __shared__ int max_info[seqs_per_block][4];//score, row, col, stop
	// max_info[seqId_block][0] = h0;
	// max_info[seqId_block][1] = -1;
	// max_info[seqId_block][2] = -1;
	// max_info[seqId_block][3] = 0;
	//----------------------------------------
	//initialize-----------------------------
	//[can be optimized. we know h0, then means know number of interatino]
	short2 initHD = make_short2(0, 0);
	for(i = 0; i < MAX_QUERY_LEN / threads_per_pair; ++i){
	...
	// if(tid == 0){
	// 	for(int d = 0; d < 50; ++d){
	// 		printf("%3d", device_bottom[seqid * MAX_QUERY_LEN + d].x);
	// 	}
	// 	for(int d = 0; d < 50; ++d){
	// 		printf("%3d", device_bottom[seqid * MAX_QUERY_LEN + d].y);
	// 	}
	// 	printf("\n");
	// }
	...
	__syncwarp();
	//---------------------------------------
	//if(chunk_idx == 0 && tid == 0) p[1] = h0 and full_chunk_num == 0 && tid == 0 p[1] = h0.
	//if(global_col == 0) local_start~local_end. h[i] = h0 -_cudaGapOE - _cudaGapExtend*(i)
	//for full chunks
	for(chunk_idx = 0; chunk_idx < full_chunk_num; ++chunk_idx){
		...
		//----------------------------
		for(int c = 0; c < 8; ++c){
			partial_bottom[seqId_block][tid*8 + c] = device_bottom[seqid*MAX_QUERY_LEN +  tid*8 + c];
			partial_bottom[seqId_block][(tid + threads_per_pair)*8 + c] = device_bottom[seqid*MAX_QUERY_LEN +  (tid + threads_per_pair)*8 + c];
		}
		//-------------------------	
		...
		for(step = 0; step < threads_per_pair + query_batch_regs -1;){
			if(tid <= step && step < tid + query_batch_regs){
				//-------------initialize-------------------------
				...
				register uint32_t rpac =packed_query_batch[packed_query_batch_idx + col_block];//load 8 bases from query_batch sequence
				//fill 8x8cell
				col_cell = 0;
...
//#pragma unroll 8
					for (l = 28 - 4*local_start, m = 1 + local_start; m < 1 + (local_end + 1); l -= 4, m++) {
						//if(seqid == 0) printf("l : %d\n", l);
						//modify M.
						CORE_LOCAL_COMPUTE();
						// if(global_col < 200)
						// 	debug[m-1][global_col] = h[m];
						//check z-drop
						row_max[m] = row_max[m] > h[m] ? row_max[m] : h[m];
						row_max_col[m] = row_max[m] > h[m] ? row_max_col[m] : global_col;
					}
					p[m] = h[m-1];
					...
					//---------------------------------------------
					maxXY_x = (prev_maxHH < maxHH) ? (col_block * 8 + col_cell) : maxXY_x;//end position on query_batch sequence corresponding to current maximum score
		...
		//short4 = score, row, col, early_stop
		// if(tid == 0 && print_debug){
		// 	printf("initial\n");
		// 	for(int g = 0; g < threads_per_pair + 1; ++g){
		// 		printf("%4d %4d %4d %4d\n", thread_max[seqId_block][g].x, thread_max[seqId_block][g].y, thread_max[seqId_block][g].z, thread_max[seqId_block][g].w);
		// 	}
		// 	printf("\n");
		// 	printf("-------------------------------------------------------\n");
		// }
		...
		for(m = 1; m < 9; ++m){
			...
			if(row_max[m] > thread_max[seqId_block][tid].x){
				...
				int max_off = (global_row + m -1) - row_max_col[m];
		...
		// if(tid == 0 && print_debug){
		// 	printf("after check early and z-drop and accum max\n");
		// 	for(int g = 0; g < threads_per_pair + 1; ++g){
		// 		printf("%4d %4d %4d %4d\n", thread_max[seqId_block][g].x, thread_max[seqId_block][g].y, thread_max[seqId_block][g].z, thread_max[seqId_block][g].w);
		// 	}
		// 	printf("\n");
		...
		// for(int thread = 0; thread < threads_per_pair; ++thread){
		// 	if(thread == tid){
		// 		for(m = 1; m < 9; ++m){
		// 			if(max_info[seqId_block][3]) break;
		// 			if(row_max[m] == 0) max_info[seqId_block][3] = 1;
		// 			if(row_max[m] > max_info[seqId_block][0]){
		// 				max_info[seqId_block][0] = row_max[m];
		// 				max_info[seqId_block][1] = global_row + m - 1;
		// 				max_info[seqId_block][2] = row_max_col[m];
		// 			}
		// 			//check z-dropoff
		// 			//[max_info[1]][max_info[2]]~[global_row+m-1][row_max_col[m]]
		// 			if(max_info[seqId_block][1]-(global_row + m - 1) > max_info[seqId_block][2] - row_max_col[m]){
		// 				if(max_info[seqId_block][0] - row_max[m] -((max_info[seqId_block][1]-(global_row + m - 1))- (max_info[seqId_block][2] - row_max_col[m]))*_cudaGapExtend > z_drop){
		// 					max_info[seqId_block][3] = 1;
		// 				}
		// 			}
		// 			else{
		// 				if(max_info[seqId_block][0] - row_max[m] -((max_info[seqId_block][2] - row_max_col[m]) - (max_info[seqId_block][1]-(global_row + m - 1)))*_cudaGapExtend > z_drop){
		// 					max_info[seqId_block][3] = 1;
		// 				}
		// 			}
		// 		}
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
		...
		for(step = 0; step < last_chunk_height + query_batch_regs -1;){
			//if(max_info[seqId_block][3]) break;
			if(thread_max[seqId_block][0].w < threads_per_pair) break;
			if(tid <= step && step < tid + query_batch_regs && tid < last_chunk_height){
				//printf("last chunk\n");
				//read ref and qeury regs
				register uint32_t gpac =packed_target_batch[packed_target_batch_idx + threads_per_pair * chunk_idx + tid];//load 8 packed bases from target_batch sequence
				register uint32_t rpac =packed_query_batch[packed_query_batch_idx + col_block];//load 8 bases from query_batch sequence
...
//#pragma unroll 8
					...
					/*
					if(tid == 0){
						printf("[%10d]", global_col);
						for(int z = 1; z < 9;++z) printf("%4d", h[z]);
						printf("\n");
					}
					*/
					// for(int z = 1; z < 9;++z)
					// 	debug[z-1][global_col] = h[z];
					if(global_col == read_len - 1){
						is_end_last = 1;
						break;
					}
		...
		// for(int thread = 0; thread < threads_per_pair; ++thread){
		// 	if(thread == tid && thread < last_chunk_height){
		// 		for(m = 1; m < 9; ++m){
		// 			if(max_info[seqId_block][3]) break;
		// 			if(row_max[m] == 0) max_info[seqId_block][3] = 1;
		// 			if(row_max[m] > max_info[seqId_block][0]){
	...
	//printf("\n");
	if(seqid == 1){
		printf("[%d] <%d> %d %d %d\n", seqid + 1, tid, maxHH, maxXY_x, maxXY_y);
	}
	*/
	...
	// __shared__ int max[seqs_per_block][threads_per_pair];
	// __shared__ int idx[seqs_per_block][threads_per_pair];
	// max[seqId_block][tid] = maxHH;
	// idx[seqId_block][tid] = tid;
	// for(int power = 2; power <= threads_per_pair; power *= 2){
	// 	if(tid % power == 0){
	// 		idx[seqId_block][tid] = max[seqId_block][idx[seqId_block][tid]] > max[seqId_block][idx[seqId_block][tid + power/2]] ? idx[seqId_block][tid] : idx[seqId_block][tid + power/2];
	// 		//max[seqId_block][tid] = max[seqId_block][tid] > max[seqId_block][tid + power/2] ? max[seqId_block][tid] : max[seqId_block][tid + power/2];
	// 	}
	// }
