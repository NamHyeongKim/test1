...
__global__ void gasal_local_kernel(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, int n_tasks, short2 *device_bottom, int *dev_h0, int band_width, int z_drop)
{
	...
	short2 *ptr_partial = &(partial_bottom[seqId_block][0]);
	short2 *ptr_device = &(device_bottom[seqid*MAX_QUERY_LEN]);
	int *ptr_row_max = &(row_max[seqId_block][tid][0]);
	int *ptr_row_max_col = &(row_max_col[seqId_block][tid][0]);
	//----------------------------------------
	//initialize-----------------------------
	...
	for(i = 0; i < (int)(read_len / threads_per_pair) + 2; ++i){
		...
		h = h > 0 ? h : 0;
		initHD.x = h;
		ptr_device[i * threads_per_pair + tid] = initHD; //[i*threads_per_pair + tid]
	}
	short4 SRCE;
	...
	for(chunk_idx = 0; chunk_idx < full_chunk_num; ++chunk_idx){
		...
		//----------------------------
		for(i = 0; i < 8; ++i){
			ptr_partial[tid*8 + i] = ptr_device[tid*8 + i];
			ptr_partial[(tid + threads_per_pair)*8 + i] = ptr_device[(tid + threads_per_pair)*8 + i];
		}
		//-------------------------	
		...
		for(step = 0; step < threads_per_pair + query_batch_regs -1;){
			if(tid <= step && step < tid + query_batch_regs /*&& band_start <= global_row + 7*/){
				...
				for (k = 28; k >= 0; k -= 4, ++col_cell, ++global_col, ++band_start, ++band_end) {
					//--------------decide row range--------------
					if(band_end < global_row || global_row + 7 < band_start) {
						p[1] = ptr_partial[(col_block % (threads_per_pair * 2))*8 + col_cell].x;
						continue;
					}
					local_start = band_start - global_row;
					local_end = band_end - global_row;
					...
					HD = ptr_partial[(col_block % (threads_per_pair * 2))*8 + col_cell];
					h[0] = HD.x;
					e = HD.y;
...
//#pragma unroll 8
					for (l = 28 - 4*local_start, m = 1 + local_start; m < 1 + (local_end + 1); l -= 4, m++) {
						CORE_LOCAL_COMPUTE();
						ptr_row_max[m - 1] = ptr_row_max[m - 1] > h[m] ? ptr_row_max[m - 1] : h[m];
						ptr_row_max_col[m - 1] = ptr_row_max[m - 1] > h[m] ? ptr_row_max_col[m - 1] : global_col;
					}
					p[m] = h[m-1];
					...
					HD.x = h[m-1];
					HD.y = e;
					ptr_partial[(col_block % (threads_per_pair * 2))*8 + col_cell] = HD;
			...
			if(step_q >= 2 && step_r == 0){
				...
				for(i = 0; i < 8; ++i){
					ptr_device[offset_device_bottom + i] = ptr_partial[(offset_partial_bottom + tid)*8 + i];
				}
				//---------------------------------
				for(i = 0; i < 8; ++i){
					ptr_partial[(offset_partial_bottom + tid)*8 + i] = ptr_device[offset_device_bottom + threads_per_pair*2*8 + i];
				}
				//--------------------------------
			}
		...
		for(m = 1; m < 9; ++m){
			if(ptr_row_max[m - 1] > thread_max[seqId_block][tid+1].x){
				thread_max[seqId_block][tid+1].x = ptr_row_max[m - 1];
				thread_max[seqId_block][tid+1].y = global_row + m -1;
				thread_max[seqId_block][tid+1].z = ptr_row_max_col[m - 1];
			}
		}
		...
		for(m = 1; m < 9; ++m){
			...
			if(ptr_row_max[m - 1] == 0){
				thread_max[seqId_block][tid].w = tid;
				break;
			}
			if(ptr_row_max[m - 1] > thread_max[seqId_block][tid].x){
				thread_max[seqId_block][tid].x = ptr_row_max[m - 1];
				thread_max[seqId_block][tid].y = global_row + m - 1;
				thread_max[seqId_block][tid].z = ptr_row_max_col[m - 1];

				i = (global_row + m -1) - ptr_row_max_col[m - 1];
				if(i < 0) i = -i;
				thread_max_off[seqId_block][tid+1] = thread_max_off[seqId_block][tid+1] > i ? thread_max_off[seqId_block][tid+1] : i;
			}
			//z-drop
			//thread_max[seqId_block][tid].y,z ~ (global_row+m-1)~ptr_row_max_col[m - 1];
			if((thread_max[seqId_block][tid].y - (global_row + m -1)) > (thread_max[seqId_block][tid].z - ptr_row_max_col[m - 1])){
				if(thread_max[seqId_block][tid].x - ptr_row_max[m - 1] - ((thread_max[seqId_block][tid].y - (global_row + m -1)) - (thread_max[seqId_block][tid].z - ptr_row_max_col[m - 1]))*_cudaGapExtend > z_drop){
					thread_max[seqId_block][tid].w = tid;
					break;
				}
			}
			else{
				if(thread_max[seqId_block][tid].x - ptr_row_max[m - 1] - ((thread_max[seqId_block][tid].z - ptr_row_max_col[m - 1]) - (thread_max[seqId_block][tid].y - (global_row + m -1)))*_cudaGapExtend > z_drop){
					thread_max[seqId_block][tid].w = tid;
					break;
				}
			}
...
//#pragma unroll 8
			...
			if(step_q >= 2 && step_r == 0){
				...
				offset_device_bottom = (offset_device_bottom + tid) * 8;
				for(i = 0; i < 8; ++i){
					ptr_partial[(offset_partial_bottom + tid)*8 + i] = device_bottom[offset_device_bottom + threads_per_pair*2*8 + i];
				}
			}
