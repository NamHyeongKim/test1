...
__global__ void gasal_local_kernel(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, int n_tasks, short2 *device_bottom, int *dev_h0, int band_width, int z_drop)
{
	...
	int32_t i, k, m, l; //[remove i->m]
	int32_t e;
	int32_t subScore;
	//----------------------------------------
	uint32_t full_chunk_num = target_batch_regs / threads_per_pair; //[remove]
	uint32_t last_chunk_height = target_batch_regs % threads_per_pair; //[remove]
	uint32_t chunk_idx; 
	uint32_t step;
	uint32_t col_block;
	uint32_t col_cell;
	...
	int step_q;	//[remove]
	int step_r;	//[remove]
	int offset_partial_bottom; //[remove]
	int offset_device_bottom; //[remove]
	...
	for(chunk_idx = 0; chunk_idx < full_chunk_num; ++chunk_idx){
		...
		for (m = 0; m < 9; m++) {
            ...
            p[m] = 0;
			//row_max[m] = 0;
			//row_max_col[seqId_block][(m-1)*threads_per_pair + tid] = 0;
        }
		for(m = 1; m < 9; ++m){
			row_max[seqId_block][(m-1)*threads_per_pair + tid] = 0;
			row_max_col[seqId_block][(m-1)*threads_per_pair + tid] = 0;
		}
		//----------------------------
		for(int c = 0; c < 8; ++c){
...
//#pragma unroll 8
					for (l = 28 - 4*local_start, m = 1 + local_start; m < 1 + (local_end + 1); l -= 4, m++) {
						...
						CORE_LOCAL_COMPUTE();
						row_max[seqId_block][(m-1)*threads_per_pair + tid] = row_max[seqId_block][(m-1)*threads_per_pair + tid] > h[m] ? row_max[seqId_block][(m-1)*threads_per_pair + tid] : h[m];
						row_max_col[seqId_block][(m-1)*threads_per_pair + tid] = row_max[seqId_block][(m-1)*threads_per_pair + tid] > h[m] ? row_max_col[seqId_block][(m-1)*threads_per_pair + tid] : global_col;
						//row_max[seqId_block][(m-1)*threads_per_pair + tid]
					}
					p[m] = h[m-1];
		...
		for(m = 1; m < 9; ++m){
			if(row_max[seqId_block][(m-1)*threads_per_pair + tid] > thread_max[seqId_block][tid+1].x){
				thread_max[seqId_block][tid+1].x = row_max[seqId_block][(m-1)*threads_per_pair + tid];
				thread_max[seqId_block][tid+1].y = global_row + m -1;
				thread_max[seqId_block][tid+1].z = row_max_col[seqId_block][(m-1)*threads_per_pair + tid];
			}
		}
		...
		for(m = 1; m < 9; ++m){
			...
			if(row_max[seqId_block][(m-1)*threads_per_pair + tid] == 0){
				thread_max[seqId_block][tid].w = tid;
				break;
			}
			if(row_max[seqId_block][(m-1)*threads_per_pair + tid] > thread_max[seqId_block][tid].x){
				thread_max[seqId_block][tid].x = row_max[seqId_block][(m-1)*threads_per_pair + tid];
				thread_max[seqId_block][tid].y = global_row + m - 1;
				thread_max[seqId_block][tid].z = row_max_col[seqId_block][(m-1)*threads_per_pair + tid];

				int max_off = (global_row + m -1) - row_max_col[seqId_block][(m-1)*threads_per_pair + tid];
			...
			//z-drop
			//thread_max[seqId_block][tid].y,z ~ (global_row+m-1)~row_max_col[seqId_block][(m-1)*threads_per_pair + tid];
			if((thread_max[seqId_block][tid].y - (global_row + m -1)) > (thread_max[seqId_block][tid].z - row_max_col[seqId_block][(m-1)*threads_per_pair + tid])){
				if(thread_max[seqId_block][tid].x - row_max[seqId_block][(m-1)*threads_per_pair + tid] - ((thread_max[seqId_block][tid].y - (global_row + m -1)) - (thread_max[seqId_block][tid].z - row_max_col[seqId_block][(m-1)*threads_per_pair + tid]))*_cudaGapExtend > z_drop){
					thread_max[seqId_block][tid].w = tid;
					break;
				}
			}
			else{
				if(thread_max[seqId_block][tid].x - row_max[seqId_block][(m-1)*threads_per_pair + tid] - ((thread_max[seqId_block][tid].z - row_max_col[seqId_block][(m-1)*threads_per_pair + tid]) - (thread_max[seqId_block][tid].y - (global_row + m -1)))*_cudaGapExtend > z_drop){
					thread_max[seqId_block][tid].w = tid;
					break;
				}
			}
