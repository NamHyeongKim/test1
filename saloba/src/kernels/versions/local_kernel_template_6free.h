...
#define CORE_LOCAL_COMPUTE() \
		...
		int16_t tmp_hm = p[m] ? p[m] + subScore : 0; \
		h[m] = max(tmp_hm, f[m]); \
		h[m] = max(h[m], e); \
		h[m] = max(h[m], 0); \
...
__global__ void gasal_local_kernel(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, int n_tasks, short2 *device_bottom, int *dev_h0, int band_width, int z_drop)
{
	...
	__shared__ int32_t row_max[seqs_per_block][threads_per_pair * 8];
	__shared__ int32_t row_max_col[seqs_per_block][threads_per_pair * 8];
	__shared__ short4 thread_max[seqs_per_block][threads_per_pair + 1]; 
	__shared__ int32_t thread_max_off[seqs_per_block][threads_per_pair + 1];
	__shared__ short2 thread_g[seqs_per_block][threads_per_pair + 1];
	__shared__ uint32_t bases[seqs_per_block][threads_per_pair * 2];

	__shared__ int32_t local[seqs_per_block][threads_per_pair][8][2];
	__shared__ bool pass[seqs_per_block][threads_per_pair][8];
	//----------------------------------------
	//initialize-----------------------------
	...
	for(chunk_idx = 0; chunk_idx < full_chunk_num; ++chunk_idx){
		...
		for(step = 0; step < threads_per_pair + query_batch_regs -1;){
			if(tid <= step && step < tid + query_batch_regs /*&& band_start <= global_row + 7*/){
				//-------------initialize-------------------------
				for(i = 0; i < 8; ++i){
					band_start = (global_col + i - band_width);
					band_end = (global_col + i + band_width);
					pass[seqId_block][tid][i] = ((band_end) < global_row || global_row + 7 < (band_start));
					local[seqId_block][tid][i][0] = (band_start - global_row) > 0 ? (band_start - global_row) : 0;
					local[seqId_block][tid][i][1] = (band_end - global_row) > 7 ? 7 : (band_end - global_row);
				}	
				...
				for (k = 28; k >= 0; k -= 4, ++col_cell, ++global_col /*,++band_start, ++band_end*/) {
					//--------------decide row range--------------
					if(pass[seqId_block][tid][col_cell]){
						p[1] = partial_bottom[seqId_block][(col_block % (threads_per_pair * 2))*8 + col_cell].x;
						continue;
					}
					local_start =  local[seqId_block][tid][col_cell][0];
					local_end = local[seqId_block][tid][col_cell][1];
					// if(band_end < global_row || global_row + 7 < band_start) {
					// 	p[1] = partial_bottom[seqId_block][(col_block % (threads_per_pair * 2))*8 + col_cell].x;
					// 	continue;
					// }
