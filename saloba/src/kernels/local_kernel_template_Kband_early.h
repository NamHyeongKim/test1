#ifndef __LOCAL_KERNEL_TEMPLATE2__
#define __LOCAL_KERNEL_TEMPLATE2__
...
__global__ void gasal_local_kernel_Kband_early(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, int n_tasks, short2 *device_bottom, int *dev_h0, int band_width, int z_drop)
{

	//return;

	int32_t seqid = blockIdx.x * blockDim.y * (blockDim.x / threads_per_pair) + threadIdx.y * (blockDim.x / threads_per_pair) + threadIdx.x / threads_per_pair;
	int32_t tid = threadIdx.x % threads_per_pair;
	if(seqid >= n_tasks) return;
	int32_t seqId_block = threadIdx.y * blockDim.x / threads_per_pair + threadIdx.x / threads_per_pair;
	...
	int32_t subScore;
	//----------------------------------------
	int32_t full_chunk_num = target_batch_regs / threads_per_pair; //[remove]
	int32_t last_chunk_height = target_batch_regs % threads_per_pair; //[remove]
	int32_t chunk_idx; 
	int32_t step;
	int32_t col_block;
	int32_t col_cell;
