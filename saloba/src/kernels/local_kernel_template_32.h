#ifndef __LOCAL_KERNEL_TEMPLATE32__
#define __LOCAL_KERNEL_TEMPLATE32__
...
enum subwarp_32{
	threads_per_pair_32 = 32,
	seqs_per_block_32 = 32 * threadIdx_y / threads_per_pair_32,
};

//template <typename T, typename S, typename B>
__global__ void gasal_local_kernel_32(uint32_t *packed_query_batch, uint32_t *packed_target_batch,  uint32_t *query_batch_lens, uint32_t *target_batch_lens, uint32_t *query_batch_offsets, uint32_t *target_batch_offsets, gasal_res_t *device_res, int n_tasks, short2 *device_bottom, int *dev_h0, int band_width, int z_drop)
{

	//return;

	int32_t seqid = blockIdx.x * blockDim.y * (blockDim.x / threads_per_pair_32) + threadIdx.y * (blockDim.x / threads_per_pair_32) + threadIdx.x / threads_per_pair_32;
	int32_t tid = threadIdx.x % threads_per_pair_32;
	if(seqid >= n_tasks) return;
	int32_t seqId_block = threadIdx.y * blockDim.x / threads_per_pair_32 + threadIdx.x / threads_per_pair_32;
	...
	int32_t subScore;
	//----------------------------------------
	int32_t full_chunk_num = target_batch_regs / threads_per_pair_32; //[remove]
	int32_t last_chunk_height = target_batch_regs % threads_per_pair_32; //[remove]
	int32_t chunk_idx; 
	int32_t step;
	int32_t col_block;
	int32_t col_cell;
	...
	__shared__ short2 partial_bottom[seqs_per_block_32][threads_per_pair_32 * 2 * 8];
	__shared__ uint32_t bases[seqs_per_block_32][threads_per_pair_32 * 2];

	__shared__ int row_max[seqs_per_block_32][threads_per_pair_32 * 8];
	__shared__ int row_max_col[seqs_per_block_32][threads_per_pair_32 * 8];

	__shared__ short4 thread_max[seqs_per_block_32][threads_per_pair_32 + 1]; 
	__shared__ int thread_max_off[seqs_per_block_32][threads_per_pair_32 + 1];

	__shared__ short2 thread_g[seqs_per_block_32][threads_per_pair_32 + 1];
	//----------------------------------------
	//initialize-----------------------------
	//[can be optimized. we know h0, then means know number of interatino]
	short2 initHD = make_short2(0, 0);
	for(i = 0; i < (int)(read_len / threads_per_pair_32) + 2; ++i){
		int h = h0 - _cudaGapOE - _cudaGapExtend * (i*threads_per_pair_32 + tid);
		h = h > 0 ? h : 0;
		initHD.x = h;
		device_bottom[seqid * MAX_QUERY_LEN + i * threads_per_pair_32 + tid] = initHD; //[i*threads_per_pair_32 + tid]
	}
	short4 SRCE;
	thread_max[seqId_block][tid] = make_short4(0, 0, 0, threads_per_pair_32);
	if(tid ==0){
		thread_max[seqId_block][0].x = h0;
		thread_max[seqId_block][0].y = -1;
		thread_max[seqId_block][0].z = -1;
		thread_max[seqId_block][0].w = threads_per_pair_32;
	}
	...
	for(chunk_idx = 0; chunk_idx < full_chunk_num; ++chunk_idx){
		...
		for(int c = 0; c < 8; ++c){
			...
			partial_bottom[seqId_block][(tid + threads_per_pair_32)*8 + c] = device_bottom[seqid*MAX_QUERY_LEN +  (tid + threads_per_pair_32)*8 + c];
		}
		//-------------------------	
		global_row = 8 * threads_per_pair_32 * chunk_idx + 8 * tid;
		global_col = 0;
		...
		__syncwarp();
		//-------------------------
		register uint32_t gpac =packed_target_batch[packed_target_batch_idx + threads_per_pair_32 * chunk_idx + tid];//load 8 packed bases from target_batch sequence
		for(step = 0; step < threads_per_pair_32 + query_batch_regs -1;){
			if(tid <= step && step < tid + query_batch_regs /*&& band_start <= global_row + 7*/){
				...
				register uint32_t rpac = bases[seqId_block][col_block % (2*threads_per_pair_32)];//load 8 bases from query_batch sequence
				#pragma unroll 8
				for (k = 28; k >= 0; k -= 4, ++col_cell, ++global_col, ++band_start, ++band_end) {
					//--------------decide row range--------------
					if(band_end < global_row /*|| global_row + 7 < band_start*/) {
						p[1] = partial_bottom[seqId_block][(col_block % (threads_per_pair_32 * 2))*8 + col_cell].x;
						continue;
					}
					...
					HD = partial_bottom[seqId_block][(col_block % (threads_per_pair_32 * 2))*8 + col_cell];
					h[0] = HD.x;
					e = HD.y;
					//--------------------------------------------
					#pragma unroll 8
					...
					HD.x = h[m-1];
					HD.y = e;
					partial_bottom[seqId_block][(col_block % (threads_per_pair_32 * 2))*8 + col_cell] = HD;
			...
			++step;
			step_q = (step) / threads_per_pair_32;
			step_r = (step) % threads_per_pair_32;
			if(step_q >= 2 && step_r == 0){
				offset_partial_bottom = (step_q % 2) * threads_per_pair_32;
				offset_device_bottom = (step_q -2) * threads_per_pair_32;
				...
				//---------------------------------
				for(i = 0; i < 8; ++i){
					partial_bottom[seqId_block][(offset_partial_bottom + tid)*8 + i] = device_bottom[seqid * MAX_QUERY_LEN + offset_device_bottom + threads_per_pair_32*2*8 + i];
				}
				//--------------------------------
			}
			if(step % (threads_per_pair_32) == 0){
				//((step/threads_per_pair_32)/2)
				bases[seqId_block][((step/threads_per_pair_32)%2)*threads_per_pair_32 + tid] = packed_query_batch[packed_query_batch_idx + step + tid];
			}
			__syncwarp();
		...
		for(i = 1; i < threads_per_pair_32; i = i * 2){
		...
		//kogge-stone algorithm for max_off. target idx is 1<= < threads_per_pair_32+1
		for(i = 1; i < threads_per_pair_32 + 1; i = i * 2){
			int bigger = 0;
			if(i-1 <= tid){
		...
		if(tid == 0){
			//max
			int idx = thread_max[seqId_block][threads_per_pair_32-1].w == threads_per_pair_32 ? threads_per_pair_32 -1 : thread_max[seqId_block][threads_per_pair_32-1].w;
		...
		if(thread_max[seqId_block][0].w < threads_per_pair_32) break;
