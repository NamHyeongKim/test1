...
/*
			...
			gasal_local_kernel<Int2Type<LOCAL>, Int2Type<s>, Int2Type<b>><<<grid, block, 0, gpu_storage->str>>>(gpu_storage->packed_query_batch, gpu_storage->packed_target_batch, gpu_storage->query_batch_lens, gpu_storage->target_batch_lens, gpu_storage->query_batch_offsets, gpu_storage->target_batch_offsets, gpu_storage->device_res, actual_n_alns, (short2 *)gpu_storage->device_bottom, gpu_storage->h0, gpu_storage->w, gpu_storage->zdrop); \
...
#define SWITCH_LOCAL(a,s,h,t,b) \
		case s: {\
			...
			float time;\
			cudaEventElapsedTime(&time, start, end);\
			FILE *fp = fopen("result/time.txt", "a");\
			fprintf(fp, "%.3f\n", time);\
			fclose(fp);\
			if(s == WITH_TB) {\
