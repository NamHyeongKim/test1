...
void gasal_init_streams(gasal_gpu_storage_v *gpu_storage_vec,  int max_query_len, int max_target_len, int max_n_alns,  Parameters *params) {
	...
	for (i = 0; i < gpu_storage_vec->n; i++) {
		...
		//
		//fprintf(stderr, "########################### %d %d ##########################\n", max_query_len, max_n_alns);
		//short2 = 4byte.  * maxquery_len * max_nums
		//should alloc 32 times for simplifing initialization of device bottom at algin kernel.
		//In this case, I alloc (max_alns+1)
		CHECKCUDAERROR(cudaMalloc(&(gpu_storage_vec->a[i].device_bottom), 4 * (32 * 1100) * (max_n_alns+1)));
		//CHECKCUDAERROR(cudaMalloc(&(gpu_storage_vec->a[i].device_bottom), 4 * (32 * 725) * (max_n_alns+1)));

		//cudaMemset((gpu_storage_vec->a[i].device_bottom), 0, 4 * (32 * 725) * (max_n_alns+1));
		...
		CHECKCUDAERROR(cudaHostAlloc(&(gpu_storage_vec->a[i].host_h0), host_max_n_alns * sizeof(uint32_t), cudaHostAllocDefault));
		...
		CHECKCUDAERROR(cudaMalloc(&(gpu_storage_vec->a[i].h0), gpu_max_n_alns * sizeof(uint32_t)));
...
void gasal_destroy_streams(gasal_gpu_storage_v *gpu_storage_vec, Parameters *params) {
	...
	for (i = 0; i < gpu_storage_vec->n; i ++) {
		...
		cudaFree(gpu_storage_vec->a[i].device_bottom);
		...
		if (gpu_storage_vec->a[i].host_h0 != NULL) CHECKCUDAERROR(cudaFreeHost(gpu_storage_vec->a[i].host_h0));
		...
		if (gpu_storage_vec->a[i].h0 != NULL) CHECKCUDAERROR(cudaFree(gpu_storage_vec->a[i].h0));
...
void gasal_gpu_mem_alloc(gasal_gpu_storage_t *gpu_storage, int gpu_max_query_batch_bytes, int gpu_max_target_batch_bytes, int gpu_max_n_alns, Parameters *params) {
	...
	CHECKCUDAERROR(cudaMalloc(&(gpu_storage->h0), gpu_max_n_alns * sizeof(uint32_t)));
...
void gasal_gpu_mem_free(gasal_gpu_storage_t *gpu_storage, Parameters *params) {
	...
	if (gpu_storage->h0 != NULL) CHECKCUDAERROR(cudaFree(gpu_storage->h0));
