...
void gasal_aln_async(gasal_gpu_storage_t *gpu_storage, const uint32_t actual_query_batch_bytes, const uint32_t actual_target_batch_bytes, const uint32_t actual_n_alns, Parameters *params, int selecting) {
	...
	CHECKCUDAERROR(cudaMemcpyAsync(gpu_storage->h0, gpu_storage->host_h0, actual_n_alns * sizeof(uint32_t), cudaMemcpyHostToDevice,  gpu_storage->str));
	...
	dim3 grid(ceil(actual_n_alns/(float)(seqs_per_block)));
	dim3 block(32, 4, 1);
	...
	if(timeRecord){
		...
		cudaEventRecord(start, gpu_storage->str);
	}

	if(selecting == 0){
		gasal_local_kernel_vanila<<<grid, block, 0, gpu_storage->str>>>(
		gpu_storage->packed_query_batch, gpu_storage->packed_target_batch,
		gpu_storage->query_batch_lens, gpu_storage->target_batch_lens,
		gpu_storage->query_batch_offsets, gpu_storage->target_batch_offsets,
		gpu_storage->device_res, actual_n_alns, (short2 *)gpu_storage->device_bottom,
		gpu_storage->h0, gpu_storage->w, gpu_storage->zdrop);
	}
	else if(selecting == 1){
		gasal_local_kernel_Kband<<<grid, block, 0, gpu_storage->str>>>(
		gpu_storage->packed_query_batch, gpu_storage->packed_target_batch,
		gpu_storage->query_batch_lens, gpu_storage->target_batch_lens,
		gpu_storage->query_batch_offsets, gpu_storage->target_batch_offsets,
		gpu_storage->device_res, actual_n_alns, (short2 *)gpu_storage->device_bottom,
		...
		gasal_local_kernel_Kband_early<<<grid, block, 0, gpu_storage->str>>>(
		gpu_storage->packed_query_batch, gpu_storage->packed_target_batch,
		gpu_storage->query_batch_lens, gpu_storage->target_batch_lens,
		gpu_storage->query_batch_offsets, gpu_storage->target_batch_offsets,
		gpu_storage->device_res, actual_n_alns, (short2 *)gpu_storage->device_bottom,
		...
		gasal_local_kernel_8<<<grid, block, 0, gpu_storage->str>>>(
		gpu_storage->packed_query_batch, gpu_storage->packed_target_batch,
		gpu_storage->query_batch_lens, gpu_storage->target_batch_lens,
		gpu_storage->query_batch_offsets, gpu_storage->target_batch_offsets,
		gpu_storage->device_res, actual_n_alns, (short2 *)gpu_storage->device_bottom,
		...
		gasal_local_kernel_16<<<grid, block, 0, gpu_storage->str>>>(
		gpu_storage->packed_query_batch, gpu_storage->packed_target_batch,
		gpu_storage->query_batch_lens, gpu_storage->target_batch_lens,
		gpu_storage->query_batch_offsets, gpu_storage->target_batch_offsets,
		gpu_storage->device_res, actual_n_alns, (short2 *)gpu_storage->device_bottom,
		...
		gasal_local_kernel_32<<<grid, block, 0, gpu_storage->str>>>(
		gpu_storage->packed_query_batch, gpu_storage->packed_target_batch,
		gpu_storage->query_batch_lens, gpu_storage->target_batch_lens,
		gpu_storage->query_batch_offsets, gpu_storage->target_batch_offsets,
		gpu_storage->device_res, actual_n_alns, (short2 *)gpu_storage->device_bottom,
	...
	if(timeRecord){
		cudaEventRecord(end, gpu_storage->str);
		...
		switch(selecting){
			case 0:
				fp = fopen("time_vanila.txt", "a");
				break;
			case 1:
				fp = fopen("time_Kband.txt", "a");
				break;
			case 2:
				fp = fopen("time_Kband_early.txt", "a");
				break;
			case 3:
				fp = fopen("time_Kband_early_zdrop.txt", "a");
				break;
			default:
				break;
		}
	...
	if (gpu_storage->host_res->max_off != NULL && gpu_storage->device_cpy->max_off != NULL) 
		CHECKCUDAERROR(cudaMemcpyAsync(gpu_storage->host_res->max_off, gpu_storage->device_cpy->max_off, actual_n_alns * sizeof(int32_t), cudaMemcpyDeviceToHost, gpu_storage->str));
    
	if (gpu_storage->host_res->gscore != NULL && gpu_storage->device_cpy->gscore != NULL) 
		CHECKCUDAERROR(cudaMemcpyAsync(gpu_storage->host_res->gscore, gpu_storage->device_cpy->gscore, actual_n_alns * sizeof(int32_t), cudaMemcpyDeviceToHost, gpu_storage->str));

	if (gpu_storage->host_res->max_ie != NULL && gpu_storage->device_cpy->max_ie != NULL) 
		CHECKCUDAERROR(cudaMemcpyAsync(gpu_storage->host_res->max_ie, gpu_storage->device_cpy->max_ie, actual_n_alns * sizeof(int32_t), cudaMemcpyDeviceToHost, gpu_storage->str));
