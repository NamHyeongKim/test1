...
int main(int argc, char **argv) {
	...
	fprintf(stderr, "%d %d %d %d\n", args->sa, args->sb, args->gapo, args->gape);
	...
	*/
	...
	fprintf(stderr, "[qeury_seq_mod] %d\n", query_seq_mod[0]);
	...
	for (int z = 0; z < n_threads; z++) {
		...
		*/		
		...
		gasal_init_streams(&(gpu_storage_vecs[z]), (35200 + 7) , //TODO: remove maximum_sequence_length_query
						(35200 + 7) ,
	...
	//std::ifstream file("SeedScore.txt"); // 파일 경로를 파일 이름 대신 실제 파일 경로로 바꿔주세요
    //std::string line;
			...
			if (seqs_done < n_seqs && gpu_batch_arr_idx < gpu_storage_vecs[omp_get_thread_num()].n) {
				...
				for (int i = curr_idx; seqs_done < n_seqs && j < (STREAM_BATCH_SIZE); i++, j++, seqs_done++)
				{
					...
					*/
					...
					(gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_h0[j] = H;

				}
				(gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->w = W;
				(gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->zdrop = Z;
				...
				gasal_aln_async(gpu_batch_arr[gpu_batch_arr_idx].gpu_storage, query_batch_bytes, target_batch_bytes, gpu_batch_arr[gpu_batch_arr_idx].n_seqs_batch, args, ABLATION);
			...
			while (gpu_batch_arr_idx < gpu_storage_vecs[omp_get_thread_num()].n) {//loop through all the streams and print the results
				...
				if (gasal_is_aln_async_done(gpu_batch_arr[gpu_batch_arr_idx].gpu_storage) == 0) {
					...
					for (int i = gpu_batch_arr[gpu_batch_arr_idx].batch_start; j < gpu_batch_arr[gpu_batch_arr_idx].n_seqs_batch; i++, j++) {
						...
						std::cout << "\tmaxoff=" << (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res->max_off[j] ;
						std::cout << "\tgscore=" << (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res->gscore[j] ;
						std::cout << "\tmax_ie=" << (gpu_batch_arr[gpu_batch_arr_idx].gpu_storage)->host_res->max_ie[j] ;
